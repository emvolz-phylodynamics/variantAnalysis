...
scanner1 <- function(treenexfn = NULL, min_descendants = 30 , max_descendants = 20e3, min_date = NULL, max_date = NULL , ncpu = 8
 , path_to_data = '/cephfs/covid/bham/results/phylogenetics/latest/'
...
print(paste('complete loading tree & filtering', Sys.time()) )
	...
	# copied/adapted from treestructure 
	{
		...
		descendantSids <- lapply( 1:(n+nnode), function(u) c() )
		for (u in 1:n){
			descendantTips[[u]] <- u 
...
print(paste('complete lookup variables', Sys.time()) ) 
